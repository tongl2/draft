...
def get_param_map(safetensors_path):
    return json.load(open(f'{safetensors_path}/param_name_map.json', 'r'))

def save_param_map(param_map, output_path):
    with open(f'{output_path}/param_name_map.json', 'w') as f:
...
def main():
    param_map = get_param_map(args.input_path)
    file_map = get_file_list(param_map)
    ...
    param_map = proc_param_map(param_map)
    save_param_map(param_map, args.output_path)
    print('Done!')
